...
program // Commander program
  .command('init [siteName] [template] [username]') // Specify the command and parameters required by your plugin
  .description('Initialize website with docusaurus-portfolio.') //Give the program a simple description
  .action((siteName, template, username) => {
    // Define the way that the program will respond after being called
    wrapCommand(init)(siteName, template, username); // Check out below for details but wrapCommand just catches errors when calling our plugin
  });
```
...
Here's a big block of commented code that should start you in the right direction.
...
There really isn't a great way to test the executable without running it locally a bunch of times.
